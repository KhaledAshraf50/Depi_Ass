...
where order_date >= '01/01/2023' and order_date < '01/01/2024';
...
select category_id,count(*) as "No of Products" from production.products
group by category_id;
--12: Count number of customers in each state
select state ,count(*) as "No of customers" from sales.customers
group by state
--13:Get average list price of products per brand 
select brand_id,AVG(list_price) as Average from production.products
group by brand_id
ORDER BY AVG(list_price) DESC;

--14: Show number of orders per staff
select staff_id,count(*) from sales.orders 
group by staff_id
...
select * from sales.orders 
where order_status =2 or order_status = 4; -- in(2,4)
